...
set @wh = 'AFWH9SC'
--DOES IT HAVE CALCUALTION
--SELECT ITH_SER,SER_DOC,SER_QTY FROM
--(SELECT ITH_SER FROM v_ith_tblc WHERE  ITH_FORM NOT IN ('SASTART','SA') AND ITH_DATEC<='2021-09-26' 
--AND ITH_WH IN (@wh)
...
SELECT PIS3_WONO,SUM(REQPER) TTLPERREQ FROM
	(SELECT PIS3_WONO,PIS3_MPART,SUM(PIS3_REQQT)/SIMQT REQPER FROM XPIS3 
		...
		WHERE PIS3_WONO IN (SELECT SER_DOC FROM
								(SELECT ITH_SER FROM v_ith_tblc WHERE  ITH_FORM NOT IN ('SASTART','SA') AND ITH_DATEC<='2021-09-26' AND ITH_WH IN (@wh) GROUP BY ITH_SER
									HAVING SUM(ITH_QTY)>0 ) VBEFFG
		LEFT JOIN SER_TBL ON ITH_SER=SER_ID) 
	GROUP BY PIS3_WONO
...
WHERE ISNULL(TTLPER,0)<TTLPERREQ AND isnull(SER_RMUSE_COMFG,'')!='flg_ok' AND (TTLPERREQ-TTLPER)>=1 
ORDER BY JOB