...
class SER extends CI_Controller {
	...
	function printfglabel(){
		global  $wid, $hgt, $padX, $padY,  $noseri, $ccustnm, $cmitmid, $cmitmd1, $cprodt, $cwo, $cprdline, $cprdshift, $cserqty, $csersheet, $cum, $crank;
		...
		function printTag($pdf,$myleft,$mytop){
			...
			$pdf->SetFont('Tahoma','',10);
			$pdf->Text($th_x+37.5, $th_y+28,$cprodt[2]); $pdf->Text($th_x+43.5, $th_y+28,$cprodt[1]);	$pdf->Text($th_x+49.5, $th_y+28,$cprodt[0]);
			...
			$pdf->Line($th_x +1, $th_y+65, $th_x+97.5,  $th_y+65);
			$pdf->Rect($th_x +1, $th_y+1, $wid, $hgt); 
			$pdf->SetFont('Tahoma','B',8);
			$pdf->Text($th_x +3, $th_y+69,'PT. SMT INDONESIA');
			// $pdf->Code39($th_x +32, $th_y+71.5,$noseri,0.5,4);
			
			$pdf->Text($th_x +68, $th_y+69,$noseri);
        }
		$pser = '';		
	...
	public function prc_splitplant1(){
		...
		if($this->SISCN_mod->check_Primary(array('SISCN_SER' => $coldreff)) >0){
			...
			if($this->SER_mod->updatebyId(array("SER_QTY" => 0, "SER_LUPDT" => $currrtime, "SER_USRID" => $CUSERID ), $coldreff) >0){
				$ser_insert_ok = 0;
	...
	public function validate_prep(){
		...
		if($this->SISCN_mod->check_Primary(array('SISCN_SER' => $coldreff)) >0){
			...
			if($this->SER_mod->insert($datas)>0){
				$ret3  = $this->SER_mod->updatebyId(array("SER_QTY" => $crestqty, "SER_LUPDT" => $currrtime , "SER_SHEET" => $crestsht), $coldreff);
				if($ret3>0){
		...
		if($this->SISCN_mod->check_Primary(array('SISCN_SER' => $coldreff)) >0){
			...
			if(strpos($crawtext, "|") !== false){ /// regular item
				...
				if($shouldreplace_reff){
					$ret3  = $this->SER_mod->updatebyId(array("SER_ID" => $newreff,"SER_REFNO" => $newreff,"SER_RAWTXT" => $crawtext), $coldreff);
				} else{
					$ret3  = $this->SER_mod->updatebyId(array("SER_ID" => $newreff,"SER_RAWTXT" => $crawtext), $coldreff);
				}
			...
			if($cfg_type=='KD' || $cfg_type=='ASP' || $cfg_type=='KDASP' ){ // TO NON REGULAR
				if($this->SER_mod->updatebyId(['SER_QTY' => 0, 'SER_LUPDT' => $currrtime, 'SER_USRID' => $CUSERID], $coldreff) > 0){
					...
					if($retser>0){
						...
						//end minus
						if($retmin >0 ){
							if (substr_count($colditem,"KD")==0 && substr_count($colditem,"ASP") == 0) {
								$rsactive_wh ="ARPRD1";
							} else {
	...
	public function qccancelscan(){
		...
		if(count($rsith)>0){			
			...
			if($isok){
				...
				$retdelith  += $this->ITH_mod->deletebyID(['ITH_SER' => $cid, 'ITH_FORM' => 'OUT-PRD-FG']);
				if($retdelith>0){
