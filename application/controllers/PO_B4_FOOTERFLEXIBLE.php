	...
	public function print(){
		...
		if(count($rs)>13){
			...
			foreach($rs as $r){
				...
				$pdf->Cell(27.5,5,number_format($r['PO_PRICE']),0,0,'R');
				$pdf->Cell(27.5,5,number_format($amount),0,0,'R');
			...
			if($ttldiscount_price>0){
				...
				$pdf->Cell(15,5,'',0,0,'C');
				$pdf->Cell(25,5,'',0,0,'R');
				$pdf->Cell(27.5,5, "(".number_format($ttldiscount_price).")" ,0,0,'R');
				$pdf->Cell(27.5,5, "(".number_format($ttldiscount_price).")",0,0,'R');
			}
			#footer
			$pdf->SetXY(6,180-$_y);
			...
			$pdf->Cell(27.5,5,number_format($total_amount),1,0,'R');
			$pdf->SetXY(6,185-$_y);
			$pdf->Cell(10,5,'',0,0,'C');
			$pdf->Cell(30,5,'',0,0,'C');
			$pdf->Cell(127.5,5,'PPN '.$ppn.'%',1,0,'L');
			$pdf->Cell(27.5,5,number_format($ppn_price),1,0,'R');
			$pdf->SetXY(6,190-$_y);
			$pdf->Cell(10,5,'',0,0,'C');
			$pdf->Cell(30,5,'',0,0,'C');				
			$pdf->Cell(127.5,5,'W/H TAX ART 23/26 '.$pph.' %',1,0,'L');
			$pdf->Cell(27.5,5,number_format($pph_price),1,0,'R');
			$pdf->SetXY(6,195-$_y);
			$pdf->Cell(10,5,'',0,0,'C');
			$pdf->Cell(30,5,'',0,0,'C');				
			$pdf->Cell(127.5,5,'DELIVERY / SHIPPING COST',1,0,'L');
			$pdf->Cell(27.5,5,number_format($shipping_cost),1,0,'R');
			$pdf->SetFont('Times','B',9);
			$pdf->SetXY(6,200-$_y);
			$pdf->Cell(10,5,'',0,0,'C');
			$pdf->Cell(30,5,'',0,0,'C');				
			$pdf->Cell(127.5,5,'NET PAYMENT ( PAY TO SUPPLIER )',1,0,'L');
			$pdf->Cell(27.5,5,number_format($netpayment),1,0,'R');
			$pdf->SetXY(6,205-$_y);
			$pdf->Cell(10,5,'',0,0,'C');
			$pdf->Cell(30,5,'',0,0,'C');				
			$pdf->Cell(127.5,5,'GRAND TOTAL',1,0,'L');
			$pdf->Cell(27.5,5,number_format($netpayment+$shipping_cost),1,0,'R');
