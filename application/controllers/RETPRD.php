...
class RETPRD extends CI_Controller
{
    ...
    public function get_psn_list()
    {
        ...
        foreach ($rs as $r) {
            $rs_j[] = [
                'id' => trim($r['RETSCN_SPLDOC']), 'text' => trim($r['RETSCN_SPLDOC'])
            ];
        }
                ...
                if (!$allow2) {
                    ...
                    if ($rs_vrs['BALQTY'] >= $cqaf) {
                        ...
                        $rsbefore = $this->SPLSCN_mod
                            ->selectby_filter([
                                'SPLSCN_DOC' => $cpsn, 'SPLSCN_CAT' => $ccat, 'SPLSCN_LINE' => $clne, 'SPLSCN_ITMCD' => $citm, 'SPLSCN_LOTNO' => $clot, 'SPLSCN_QTY' => $cqbf
                            ]);
                        #END
                        if (!empty($rsbefore)) {
        ...
        if (is_array($citm)) {
            ...
            if ($this->SPL_mod->check_Primary($dataw) > 0) {
                ...
                for ($i = 0; $i < $ttldata; $i++) {
                    ...
                    $C3Data[] = [
                        'C3LC_ITMCD' => $citm[0], 'C3LC_NLOTNO' => $lotasHome, 'C3LC_NQTY' => $cqaf, 'C3LC_LOTNO' => $clot[$i], 'C3LC_QTY' => $cqbf[$i], 'C3LC_REFF' => $newid, 'C3LC_LINE' => $i,  'C3LC_USRID' => $cuser, 'C3LC_LUPTD' => $currrtime
                    ];
                }
                if ($isItemlotOK) {
                    ...
                    if ($rs_vrs['BALQTY'] >= $cqaf) {
                        ...
                        $rsbefore = $this->SPLSCN_mod
                            ->selectby_filter([
                                'SPLSCN_DOC' => $cpsn, 'SPLSCN_CAT' => $ccat, 'SPLSCN_LINE' => $clne, 'SPLSCN_ITMCD' => $citm[0], 'SPLSCN_LOTNO' => $clot[0], 'SPLSCN_QTY' => $cqbf[0]
                            ]);
                        #END
                        if (count($rsbefore) > 0) {
    ...
    public function save_alt()
    {
        ...
        if ($allow) {
            ...
            if ($toret > 0) {
                $myar[] = [
                    'cd' => '11', 'msg' => 'Saved', "xitem" => $citm, "xqty" => $cqaf, "xlot" => $clot, "xitemnm" =>  $citemnm
                ];
                die('{"status":' . json_encode($myar) . '}');
            }
        } else {
    ...
    function auto_confirm()
    {
        ...
        die(json_encode(
            [
                ...
                [
                    'rs_last_month' => $rspsn_last_month, 'rs_current_month' => $rspsn_current_month
    ...
    public function rtn_without_psn()
    {
        ...
        if ($this->RETRM_mod->check_Primary(['RETRM_ITMCD' => $itemcd, 'RETRM_OLDQTY' => $oldqty, 'RETRM_LOTNUM' => $lotnum, 'CONVERT(DATE, RETRM_CREATEDAT)=' => $currentDate])) {
            ...
            $data = [
                'RETRM_DOC' => $doc, 'RETRM_LINE' => 1, 'RETRM_ITMCD' => $itemcd, 'RETRM_OLDQTY' => $oldqty, 'RETRM_NEWQTY' => $newqty, 'RETRM_LOTNUM' => $lotnum, 'RETRM_CREATEDAT' => $currentDateTime, 'RETRM_USRID' => $usrid
            ];
    ...
    public function rtn_without_psn_list_period()
    {
        header('Content-Type: application/json');
        $dt1 = $this->input->get('date1');
        $dt2 = $this->input->get('date2');
        $itemcd = $this->input->get('itemcd');
        $business = $this->input->get('business');
        $rs = $this->RETRM_mod->selectWithRackAndLike(
            ['CONVERT(DATE, RETRM_CREATEDAT)>=' => $dt1, 'CONVERT(DATE, RETRM_CREATEDAT)<=' => $dt2, 'ITMLOC_BG' => $business],
            ['RETRM_ITMCD' => $itemcd]
        );
        die(json_encode(['data' => $rs]));
    }
    ...
    public function cancel_rtn_without_psn()
    {
        ...
        if (count($rs) > 0) {
            if ($this->ITH_mod->check_Primary(['ITH_DOC' => $idscan])) {
                ...
                $txMonth = $txYear = null;
                $rsTransaction = $this->RETRM_mod->select_where(['YEAR(RETRM_CREATEDAT) TXYEAR', 'MONTH(RETRM_CREATEDAT) TXMONTH'], ['RETRM_DOC' => $idscan]);
                foreach ($rsTransaction as $r) {
                    $txMonth = $r['TXMONTH'];
                    $txYear = $r['TXYEAR'];
                }
                if ($this->RPSAL_INVENTORY_mod->check_Primary(['INV_MONTH' => $txMonth, 'INV_YEAR' => $txYear, 'INV_ITMNUM' => $itemcd])) {
