...
class RCV extends CI_Controller
{
    ...
    public function trigger_toITH()
    {
        ...
        $qry = "select CONCAT('DO',NOMOR_PENGAJUAN) RCV_DONO from data_machine_and_tools";
    ...
    public function remove_rtn_fg()
    {
        ...
        $cdoc = $this->input->post('indoc');
        $cline = $this->input->post('inline');
        $rs = $this->SER_mod->select_where(['SER_DOC' => $cdoc, 'SER_PRDLINE' => $cline]);
        $reffid = '';
        foreach ($rs as $r) {
            $reffid = $r['SER_ID'];
            break;
        ...
        die(json_encode(['status' => $myar, 'rs' => $rs]));
    }
    ...
    {
        header('Content-Type: application/json');
        $rsbooked = $this->RCV_mod->select_balanceEXBC_fromSCRBook([
            '21/12/04/0001',
            '21/12/04/0002',
            '21/12/04/0003',
            '21/12/04/0005',
    ...
    {
        header('Content-Type: application/json');
        $rsbooked = $this->RCV_mod->select_balanceEXBC_fromSCRBook_detail([
            '21/12/04/0001',
            '21/12/04/0002',
            '21/12/04/0003',
            '21/12/04/0005',
