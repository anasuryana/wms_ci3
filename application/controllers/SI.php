...
class SI extends CI_Controller {
		...
		if($cbg=='PSI1PPZIEP' && $ccustomer=='SME007U'){
			if(count($rs)>0){
				...
				if( $a_diff_cnt>0 ){
					...
					// $rs_mst_price =  $this->XSO_mod->select_latestprice($cbg, $ccustomer,$list) ; 2022-01-10 mega get error , so that this line of ode was used
					foreach($rs_mst_price as $r){
						$rs[] = ['SSO2_CPONO' => $cpo
								,'SSO2_ORDQT' => 1
								,'SSO2_DELQT' => 0
								,'SSO2_DELCD' => null
								,'SSO2_MDLCD' => $r['MSPR_ITMCD']
								,'SSO2_SLPRC' => substr($r['MSPR_SLPRC'],0,1) =='.' ? '0'.$r['MSPR_SLPRC'] : $r['MSPR_SLPRC']
								,'SSO2_SOLNO' => 'X'
							];
					}
				}				
					...
					if($cstrloc_u[$u] == $cstrloc[$i]){						
						...
						for($k=0;$k<$ttlplot;$k++){
							if($cp_idx[$k]==strval($i)){
								...
								$datasiso = $this->SISO_mod->insert($datap);
							}
						}
						$datas[] = [
							'SI_DOC' => $newid,
	...
	function checkSI(){
		...
		if($ttlrows>0){
			...
			$myar[] = ["cd" => $ttlrows, "msg" => "SI Doc not found"];
		}
	...
	public function search(){
		...
		if($clist=='today'){
			if($cby=='si'){
				$rs = $this->SI_mod->selectAllg_byDATEVAR($currdt, $ckey);
			} else {
				$rs = $this->SI_mod->selectAllg_byDATEVARCUS($currdt, $ckey);
			}
		} else {
			if($cby=='si'){
				$datal = ['SI_DOC' => $ckey];
	...
	public function searchoth(){
		...
		if($clist=='today'){			
			if($cby=='si'){							
				$rs = $this->SI_mod->selectAllg_byDATEVARoth($currdt, $ckey);
			} else {				
				$rs = $this->SI_mod->selectAllg_byDATEVARCUSoth($currdt, $ckey);
			}
		} else {
			if($cby=='si'){
				$rs = $this->SI_mod->selectAllg_byVARoth(['SI_DOC' => $ckey]);
			} else {
				$rs = $this->SI_mod->selectAllg_byVARoth(['MCUS_CUSNM' => $ckey	]);
			}
		}
		echo json_encode($rs);
	}
				...
				if($toret==0){
					...
					for($k=0;$k<$ttlplot;$k++){
						...
						if($cp_idx[$k]==strval($i) and $csiline[$k]==''){
							$sini2.="[$i$k] sino";
							$datap = ['SISO_HLINE' => $csi."-".($ttldata+1),
								'SISO_CPONO' => $cp_so[$k],
								'SISO_SOLINE' => $cp_soline[$k],
					...
					for($k=0;$k<$ttlplot;$k++){
						...
						$lastsisoline++;
						$sini.=" ". $cp_idx[$k]."==".$i."[".$csiline[$k]."]";
						if($cp_idx[$k]==strval($i) and $csiline[$k]==''){
							$datap = ['SISO_HLINE' => $clineno[$i],
								'SISO_CPONO' => $cp_so[$k],
								'SISO_SOLINE' => $cp_soline[$k],
	...
	public function savescan(){
		...
		$cfm_out = '';
		$dataf_txroute = [
			'TXROUTE_ID' => 'RECEIVING-FG-SHP'
			, 'TXROUTE_WH' => $_COOKIE["CKPSI_WH"]
		];
	...
	public function remove(){
		...
		$cdocreff = $this->input->get('indocreff');
		$citem = $this->input->get('initem');		
		...
		if($ttlscan>0){
			...
			if($toret>0){
				...
				$datar = ["cd" => "11", "msg" => "Deleted ".$additionalinfo , "ref" => $csi, "siline" => $line];
				...
				$datar = array("cd" => "01", "msg" => "Could not be deleted " , "ref" => $csi);
				array_push($myar, $datar);
				echo '{"data":';
				echo json_encode($myar);
				echo '}';
	...
	public function getdiscrepancyso(){
		...
		foreach($rs as $r){
			...
			if(!$isfound){
				array_push($rsheader, ['SILINE' => $r['SISO_HLINE'], 'SIQTY' => intval($r['SI_QTY'])]);
			}
		}
		foreach($rs as &$r){
			foreach($rsheader as &$h){
		...
		// die('{"header":'.json_encode($rsheader).', "detail" : '.json_encode($rs).'}');
		$myar =[];
		if(count($rs)>0){
			array_push($myar, ['cd' => '1' , 'msg' => 'go ahead']);
		} else {			
			array_push($myar, ['cd' => '0' , 'msg' => 'No discrepancy']);
		}
	...
	public function getsobyreffno(){
		...
		die('{"status": '.json_encode($myar)
			.', "data": '.json_encode($rs)
			.', "dataresume" : '.json_encode($rsresume)
			.', "datamultiprice" : '.json_encode($rsmultiprice).'}');
	}
