...
class SER extends CI_Controller
{
   ...
   public function set_rc_scrap()
   {
      ...
      if ($this->SERRC_mod->check_Primary(['SERRC_SERX' => $serx, 'SERRC_SERXRAWTXT' => ''])) {
         ...
         $data = [
            'SERRC_SER' => $serx, 'SERRC_JM' => '', 'SERRC_BOMPN' => '', 'SERRC_BOMPNQTY' => 0, 'SERRC_LOTNO' => '', 'SERRC_LINE' => $lastline, 'SERRC_LOC' => '', 'SERRC_DOCST' => $dok, 'SERRC_DOCSTDT' => $cdate, 'SERRC_JMOPT' => 'NOJM', 'SERRC_USRID' => $this->session->userdata('nama'), 'SERRC_LUPDT' => $current_time,
            'SERRC_NASSYCD' => '162563007', 'SERRC_SERX' => $serx, 'SERRC_SERXRAWTXT' => '', 'SERRC_SERXQTY' => 1
         ];
               ...
               foreach ($rs_ser_val as $s) {
                  foreach ($rs_ext_u as $r) {
                     if ($s['SER_ID'] == $r['SER_ID']) {
                        $rs_tx_in[] = [
                           'ITH_ITMCD' => $r['SER_ITMID'], 'ITH_DATE' => $current_date, 'ITH_FORM' => 'INC-RCRTN-FG', 'ITH_DOC' => $pser, 'ITH_QTY' => $r['SER_QTY'], 'ITH_WH' => 'ARRCRT', 'ITH_SER' => $r['SER_ID'], 'ITH_LUPDT' => $currrtime, 'ITH_USRID' => $this->session->userdata('nama')
                        ];
                     }
                  }
               }
