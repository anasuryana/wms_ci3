...
class SER extends CI_Controller {
	...
	public function convert_rank() {
		...
		if($this->SER_mod->updatebyId(['SER_GRADE' => $newrank],['SER_ID' => $oldreff]) ) {
			...
			$this->LOGSER_mod->insert_([
				...
				, 'LOGSER_DT' => date('Y-m-d H:i:s')
				, 'LOGSER_REMARK' => 'Change rank '.$oldrank.' to '.$newrank
			]);
		} else {
	...
	public function get_rm_eu(){
		...
		foreach($rs as $r){
			$rs_j[] = [
				'id' => $r['MITM_ITMCD']				
				,'description' => $r['MITM_ITMD1']
			];
			foreach($rs_s as $s){
				if(trim($s['MSPP_BOMPN'])==$r['MITM_ITMCD']){
					$rs_j[] = [
						'id' => $s['MSPP_SUBPN']						
						,'description' => $s['XSUBNAME']
					];
				}
			}
		}
	...
	public function searchdok_rcqc(){
		...
		switch($doctype){
			case 'ST':
				$rs = $this->SERRC_mod->select_serahterima_h_byst($doc);break;
			case 'DO':
				$rs = $this->SERRC_mod->select_serahterima_h_bydo($doc);break;
			case 'SL':
				$rs = $this->SERRC_mod->select_serahterima_h_bysl($doc);break;
		}
		$rsret = [];
		foreach($rs as &$r){
	...
	public function set_rc_bom(){
		...
		for($i=0; $i<$ttldata; $i++){
			if($line[$i]==''){
				...
				$lastline++;
				$data = ['SERRC_SER' => strtoupper($qalabel[$i])
					,'SERRC_JM' => $jmcode[$i]
					,'SERRC_BOMPN' => $bompn[$i]
					,'SERRC_BOMPNQTY' => is_numeric($cqty[$i]) ? $cqty[$i] : NULL
					,'SERRC_LOTNO' => $clotno[$i]
					,'SERRC_LINE' => $lastline
					,'SERRC_LOC' => $loc[$i]
					,'SERRC_DOCST' => $dok
					,'SERRC_DOCSTDT' => $cdate
		...
		for($i=0;$i<$ttlx_data; $i++){
			...
			// if($this->SER_mod->check_Primary(['SER_ID' => $cx_reffno[$i]])==0 ){
				$this->SERRC_mod->update_where([
					'SERRC_NASSYCD' => $cx_itemcd[$i]
					,'SERRC_SERX' => $cx_reffno[$i]
					,'SERRC_SERXRAWTXT' => $cx_rawtxt[$i]
					,'SERRC_SERXQTY' => $cx_qty[$i]
				], 
				[
					'SERRC_SER' =>  $cx_id[$i]
					,'SERRC_DOCST' => $dok
	...
	public function print_st_rcqc(){
		...
		// DIE(json_encode(['data' => $rsc, 'status' => 1, 'jmmode' => $jmmode]));
		...
		if(count($rs_ext_u)>0){						
			...
			if($bisgrup=='PSI1PPZIEP'){
				...
				// DIE(json_encode(['data' => $rsc, 'status' => 2, 'jmmode' => $jmmode]));		
				#1 filter inc
				foreach($rs_ser_tbl as $r){				
					$rs_ser_val[] = $r;				
				}
				...
				// DIE(json_encode(['data' => $rsc, 'status' => 3, 'jmmode' => $jmmode
				// ,'rs_tx_out' => $rs_tx_out, 'rs_tx_in' => $rs_tx_in]));	
				$this->ITH_mod->insertb($rs_tx_out);
				$this->ITH_mod->insertb($rs_tx_in);				
			}
		}
		// die('{"rs_ext_u":'.json_encode($rs_ext_u).',"ser":'.json_encode($rs_ser_val)
		// 	.',"inc":'.json_encode($rs_tx_in).', "out":'.json_encode($rs_tx_out).',"rsout":'.json_encode($rs_out).'}');
	...
	public function remove_rm_rc(){
		...
		$ret = $this->SERRC_mod->deletebyID(['SERRC_SER' => $qalabel
		, 'SERRC_LINE' => $line
		, 'SERRC_DOCST' => $docno
		, 'SERRC_SERX IS NULL' => NULL]);
		$myar = [];
		if($ret>0){
	...
	public function setfg_return(){
		...
		$cqty = $this->input->post('inqty');
		$cline = $this->input->post('inline');
		$pYear	= substr($cproddt,2,2); $pMonth = substr($cproddt,5,2); $pDay = substr($cproddt, -2); $pHour = date('H');
		$pMonthdis	= $this->getMonthDis($pMonth);
		$cmdl = '4';				
	...
	function print_return_control_label(){
	...
	public function properties_c_rank(){
		...
		die(
			json_encode(['data' => $rsold_reff, 'status' => $myar])
		);
	}
	...
	public function validate_relable(){
		...
		if($this->SISCN_mod->check_Primary(['SISCN_SER' => $coldreff]) >0){			
			...
			if(strpos($crawtext, "|") !== false){ /// regular item
				...
				$newreff = substr($araw[5],2,strlen($araw[5])-2);
				//log first
				$this->LOGSER_mod->insert_replace_log(["LOGSER_KEYS" => $crawtext, "LOGSER_KEYS_RPLC" => $coldreff, "LOGSER_DT" => $currrtime
				, "LOGSER_USRID" =>$this->session->userdata('nama') ]);
				//end log
	...
	public function convert_prep(){
		...
		die(json_encode(['data' => $rsold_reff, 'tx' => $rxtx, 'status' => $myar ]));		
	}
		...
		foreach($rsalike as $r){
			...
			if(count($this->SPL_mod->select_jobper_wmscal_byseruse($cjob,$r['SERD2_SER'], $r['SERD2_QTPER'] )) >0){ #$ttluse
				...
				if($this->SERD_mod->insert2([
					...
					,'SERD2_CAT' => ''
					,'SERD2_FR' => $cfr
					,'SERD2_SER' => $r['SERD2_SER']
					,'SERD2_FGQTY' => $r['SERD2_FGQTY']
					,'SERD2_JOB' => $cjob
					,'SERD2_QTPER' => $cper
					,'SERD2_MC' => $cmc
					,'SERD2_MCZ' => $cmcz
					,'SERD2_ITMCD' => $citmcd
					,'SERD2_QTY' =>  $cper*$r['SERD2_FGQTY']  #($cactqt > 0) ?: 0
					,'SERD2_REMARK' => 'MANUAL ADDED'
					,'SERD2_LOTNO' => ''
					,'SERD2_USRID' => $usrid
					,'SERD2_LUPDT' => $currtime
				])){					
					$ttladded++;
				}
			}			
	...
	public function flag_rmuse_ok(){
		...
		foreach($rsalike as $r){
			if($this->SER_mod->updatebyId(
				[
					'SER_RMUSE_COMFG' => 'flg_ok'
					,'SER_RMUSE_COMFG_DT' => $currtime
					,'SER_RMUSE_COMFG_USRID' => $userid
				], 
				$r['SERD2_SER'])>0)
			{
				$ttlupdated++;
	...
	public function upload_serd2(){
		...
		foreach($rs as $r){
			$rssave[] = [
				...
				,'SERD2_FR' => ''
				,'SERD2_SER' => $r->SER
				,'SERD2_FGQTY' => $r->DLV_QTY
				,'SERD2_JOB' => $r->job
				,'SERD2_QTPER' => $r->QTY_PER
				,'SERD2_MC' => ''
				,'SERD2_MCZ' => ''
				,'SERD2_ITMCD' => $r->MAIN_PARTS
				,'SERD2_QTY' => $r->QTY_PER*$r->DLV_QTY
				,'SERD2_LOTNO' => ''
				,'SERD2_REMARK' => ''
				,'SERD2_USRID' => 'ane'
	...
	public function resetcalculation(){
		...
		for($i=0; $i<$countser; $i++){
			$this->SERD_mod->deletebyID_label(['SERD2_SER' => $cser[$i]]);
		}
		$Calc_lib = new RMCalculator();
					...
					if(substr($r['SER_DOC'],3,1)=='C'){
						for($i=0; $i<$RowsCount; $i++){
							if($r['SER_ID']==$aReffNo[$i]){
								$rscjt[] = ['SERC_NEWID' => $r['SER_ID']
									,'SERC_COMID' => $aID[$i]
									,'SERC_COMJOB' => $aOldJob[$i]
									,'SERC_COMQTY' => $aINTQty[$i]
									,'SERC_USRID' => $userid
	...
	public function combine_rmlbl_desktop(){
		...
		if(is_array($citm)){
			...
			#END
			for($i=0; $i< $ttldata; $i++){
				$C3Data[] = ['C3LC_ITMCD' => $citm[0], 'C3LC_NLOTNO' => $lotasHome, 'C3LC_NQTY' => $newqty,'C3LC_LOTNO' => $clot[$i]
				, 'C3LC_QTY' => $cqty_com[$i], 'C3LC_REFF' => $newid, 'C3LC_LINE' => $i,  'C3LC_USRID' => $cuser, 'C3LC_LUPTD' => $currrtime];
			}
	...
	public function updatecalculation(){
		...
		$result = $this->SERD_mod->updatebyId(['SERD2_QTY' => $newqty, 'SERD2_REMARK' => 'edit qty', 'SERD2_USRID' => $cuser]
