...
class DELV extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['CSMLOG_mod', 'DLVH_mod', 'RPSAL_BCSTOCK_mod', 'WMS_DLVCHK_mod', 'DLVSCR_BB_mod']);
        ...
        if (substr($ctxid, -3) != 'RTN') {
            ...
            if ($cbg === 'PSI1PPZIEP') {
                ...
                if (!empty($rssiso) && $cconfirmation === 'n') {
                    $myar[] = ["cd" => '22', "msg" => "Month of Sales Order is different than Month of Delivery Order. Are you sure want to continue ?"];
                    die(json_encode($myar));
                }
            }
        }
    ...
    public function printdocs()
    {
        ...
        if (substr($pforms, 1, 1) == '1') {
            ...
            $ttlqty_ = 0;
            foreach ($rsinv as $r) {
                $_itemName = in_array($DLV_CONSIGN, ['IEI']) ? $r['MITM_ITMD2'] : $r['MITM_ITMD1'];
                if ($ttlbrs > $MAX_INVD_PERPAGE) {
                    $ttlbrs = 1;
                    $pdf->AddPage();
                    $pdf->SetFont('Arial', '', 8);
            ...
            if ($ASPFLAG) {
                ...
                foreach ($rspickingres as $r) {
                    ...
                    if ($par_item != $r['SI_ITMCD']) {
                        $par_item = $r['SI_ITMCD'];
                        $dis_item = $r['SI_ITMCD'];
                        $dis_itemnm = in_array($DLV_CONSIGN, ['IEI']) ? $r['MITM_ITMD2'] : $r['MITM_ITMD1'];
                        $dis_qty = number_format($r['TTLBOX'], 0);
                        $no++;
                        $dis_no = $no;
                    } else {
        ...
        if (substr($pforms, 0, 1) == '1') {
            ...
            foreach ($rs as $r) {
                ...
                if (!$isexist) {
                    $ar_item[] = $r['SER_ITMID'];
                    $ar_itemdesc[] = in_array($DLV_CONSIGN, ['IEI']) ? $r['MITM_ITMD2'] : $r['MITM_ITMD1'];
                    $ar_itemUM[] = $r['MITM_STKUOM'];
                    $ar_qty[] = $r['SISCN_SERQTY'];
                    $ar_po[] = $r['SISCN_DOCREFF'];
                }
                ...
                if (count($rsdo) > 0) {
                    ...
                    foreach ($rsdo as $n) {
                        if ($tempItem != $n['DLVSO_ITMCD']) {
                            ...
                            $tempItem = $n['DLVSO_ITMCD'];
                            $ItemDis = $tempItem;
                            $ItemDis2 = in_array($DLV_CONSIGN, ['IEI']) ? $n['MITM_ITMD2'] :  $n['MITM_ITMD1'];
                            $nourutDODis = $nourutDO;
                        } else {
                            $ItemDis = '';
                            $ItemDis2 = '';
                    ...
                    $pdf->SetXY(31, 227);
                    $pdf->Cell(70, 4, "On behalf PT. Sumitronics Indonesia", 1, 0, 'C');
                } else {
                    $ttlqty = 0;
    ...
    public function _posting41($DOC)
    {
        ini_set('max_execution_time', '-1');
        $currentDate = date('Y-m-d H:i:s');
        $csj = $DOC;
        ...
        if (count($rs_rm_null) > 0) {
            $myar = ['cd' => 100, 'msg' => 'RM is null, please check again data in the table below'];
            return ["status" => $myar, "data" => $rs_rm_null];
        }
        $ccustdate = '';
        $nomoraju = '';
        $czcurrency = '';
        ...
        try {
            ...
            foreach ($requestGroup as $k) {
                ...
                if (!is_null($rsbc)) {
                    ...
                    $myar = ["cd" => "0", "msg" => "Could not find exbc, please contact admin"];
                    return [
                        'status' => $myar, 'request_h' => ['doc_type' => $czdocbctype],
            ...
            if (count($listNeedExBC) > 0) {
                ...
                $myar = ['cd' => 110, 'msg' => 'EX-BC for ' . count($listNeedExBC) . ' item(s) is not found. ', "doctype" => $czdocbctype, "tujuankirim" => $cztujuanpengiriman];
                return [
                    'status' => $myar, 'data' => $listNeedExBC, 'rawdata' => $rstemp,
                    'czdocbctype' => $czdocbctype, 'cztujuanpengiriman' => $cztujuanpengiriman,
                    'responseTemparray' => $responseTemparray, 'requestResume' => $requestResume,
    ...
    public function _posting25($DOC)
    {
        ...
        if (empty($rsUnfinished)) {
            ...
            return ['status' => ['cd' => '130', 'msg' => 'there is another user posting this document']];
        }
        ...
        if (count($rscurr) == 0) {
            $this->set_log_finish_posting($csj);
            return ['status' => ["cd" => "0", "msg" => "Please fill exchange rate data !"]];
        } else {
            foreach ($rscurr as $r) {
                if ($czcurrency == 'RPH') {
                    $czharga_matauang = 1;
    ...
    public function confirm_delivery()
    {
        ...
        $ITHDATE = date('Y-m-d');
        $cdo = $this->input->get('indo');
        $DLVH_TRANS = $this->input->get('DLVH_TRANS');
        ...
        if ($resith > 0) {
            ...
            $this->DLVH_mod->updatebyVAR(['DLVH_TRANS' => $DLVH_TRANS], ['DLVH_ID' => $cdo]);
    ...
    public function dispose_report2()
    {
        ...
        foreach ($rsRM as &$r) {
            ...
            foreach ($rsEXBC as &$k) {
                $reqPlot = $r['QTY'] - $r['QTYPLOT'];
                // if ($r['PART_CODE'] === $k['ITMNUM'] && $r['ID_TRANS'] == $k['RPSTOCK_REMARK'] && $reqPlot > 0 && $k['BCQTY'] > 0) {
                if ($r['PART_CODE'] === $k['ITMNUM'] && $reqPlot > 0 && $k['BCQTY'] > 0) {
                    $theqty = $reqPlot;
                    if ($reqPlot > $k['BCQTY']) {
                        $r['QTYPLOT'] += $k['BCQTY'];
        ...
        if (!empty($TPBData)) {
            ...
            foreach ($rsitem_p_price as &$r) {
                ...
                $tpb_barang[] = [
                    'KODE_BARANG' => $r['SSO2_MDLCD']
                    , 'POS_TARIF' => $r['MITM_HSCD']
                    , 'URAIAN' => in_array($Consign, ['IEI']) ? $r['MITM_ITMD2'] : $r['MITM_ITMD1']
                    , 'JUMLAH_SATUAN' => $r['SISOQTY']
                    , 'KODE_SATUAN' => $r['MITM_STKUOM'] == 'PCS' ? 'PCE' : $r['MITM_STKUOM']
                    , 'NETTO' => $r['NWG']
                    , 'CIF' => round($r['CIF'], 2)
                ...
                foreach ($rsbc as &$v) {
                    if ($r['RITEMCDGR'] === $v->BC_ITEM || $r['RITEMCD'] === $v->BC_ITEM && $v->BC_QTY > 0) {
                        ...
                        if ($v->RCV_KPPBC != '-') {
                            ...
                            $_nomorAjuDokumenAsal = strlen($v->BC_AJU) == 6 ? substr('000000000000000000000000', 0, 26) : $v->BC_AJU;
                            $_isFound = false;
                            $_cif = ($v->RCV_PRPRC * $theqty);
                            $_seriBarangDokAsal = empty($v->RCV_ZNOURUT) ? 0 : $v->RCV_ZNOURUT;

                            foreach ($tpb_bahan_baku as &$_s) {

                                if ($_s['NOMOR_AJU_DOK_ASAL'] == $_nomorAjuDokumenAsal && $_s['KODE_BARANG'] == $v->BC_ITEM && $_s['SERI_BARANG_DOK_ASAL'] == $_seriBarangDokAsal
                                    && $_s['RASSYCODE'] == $r['RASSYCODE'] && $_s['RPRICEGROUP'] == $r['RPRICEGROUP']) {

                                    $_s['CIF'] += $_cif;
                                    $_s['JUMLAH_SATUAN'] += $theqty;
                                    $_isFound = true;
                                    break;
                            ...
                            if (!$_isFound) {
                                $tpb_bahan_baku[] = [
                                    ...
                                    , 'TANGGAL_DAFTAR_DOK_ASAL' => $v->BC_DATE
                                    , 'KODE_KANTOR' => $v->RCV_KPPBC
                                    , 'NOMOR_AJU_DOK_ASAL' => $_nomorAjuDokumenAsal
                                    , 'SERI_BARANG_DOK_ASAL' => $_seriBarangDokAsal
                                    , 'SPESIFIKASI_LAIN' => null
                                    , 'CIF' => $_cif
                                    , 'HARGA_PENYERAHAN' => 0
                                    , 'KODE_BARANG' => $v->BC_ITEM
                ...
                foreach ($rspickingres as $r) {
                    ...
                    if (!$isplot) {
                        $tpb_barang_temp[] = [
                            ...
                            'URAIAN' => in_array($DLV_CONSIGN, ['IEI']) ? $r['MITM_ITMD2'] : $r['MITM_ITMD1'],
                    ...
                    foreach ($rspickingres as $p) {
                        if ($n['KODE_BARANG'] == $p['SI_ITMCD'] && $n['PERPRICE'] == $p['RCV_PRPRC']) {
                            $tpb_bahan_baku[] = [
                                ...
                                , 'KODE_STATUS' => "03"
                                , 'POS_TARIF' => $p['RCV_HSCD']
                                , 'URAIAN' => in_array($DLV_CONSIGN, ['IEI']) ? $p['MITM_ITMD2'] : $p['MITM_ITMD1']
                                , 'TIPE' => $p['MITM_SPTNO']
                                , 'JUMLAH_SATUAN' => $p['PERBOX'] * $p['BOX']
                                , 'JENIS_SATUAN' => ($p['MITM_STKUOM'] == 'PCS') ? 'PCE' : $p['MITM_STKUOM']
                                , 'KODE_ASAL_BAHAN_BAKU' => ($p['RCV_BCTYPE'] == '27' || $p['RCV_BCTYPE'] == '23') ? '0' : '1'
    ...
    public function postingCEISA40BC27rm()
    {
        ...
        if (!empty($TPBData)) {
            ...
            foreach ($Packagings as $r) {
                if ($r['DLV_PKG_NWG'] == 0) {
                    $validationResult[] = 'Net weight [' . $index++ . '] should not be zero';
                }
                if ($r['DLV_PKG_GWG'] == 0) {
                    $validationResult[] = 'Gross weight [' . $index++ . '] should not be zero';
                }
            ...
            // if (empty($Packagings) || !empty($validationResult)) {
            //     $respon = [
            //         'message' => 'please fill packaging form properly',
            //         $validationResult,
            //     ];
            //     $this->output->set_status_header(400);
            //     die(json_encode($respon));
            ...
            foreach ($tpb_bahan_baku as &$r) {
                ...
                $tpb_barang[] = [
                    ...
                    'NETTO' => $SeriBarang == 1 ? $netweight_represent : 0,
                    'CIF' => $r['CIF'],
    ...
    public function postingCEISA40BC25rm()
    {
        header('Content-Type: application/json');
        $doc = $this->input->post('doc');
        ...
        if (!empty($TPBData)) {
            ...
            if ($isScrapItem) {
                ...
                foreach ($Barang as $r) {
                    ...
                    $tpb_barang[] = [
                        ...
                        'URAIAN' => $r['MITM_ITMD1'],
                        'TIPE' => '',
                        'JUMLAH_SATUAN' => $r['DLVSCR_ITMQT'],
                        'KODE_SATUAN' => $r['MITM_STKUOM'],
                        'NETTO' => $SeriBarang == 1 ? $netweight_represent : 0,
                        'CIF' => $_cifBarang,
                ...
                foreach ($rs_xbc as $r) {
                    $tpb_bahan_baku[] = [
                        ...
                        'CIF' => 0,
                        'HARGA_PENYERAHAN' => 0,
                        'KODE_BARANG' => $r['DLVSCR_BB_ITMID'] ?? '-',
                        'KODE_STATUS' => "03",
                        'POS_TARIF' => $r['DLVSCR_BB_HSCD'],
                        'URAIAN' => $r['DLVSCR_BB_ITMD1'],
                        'TIPE' => '-',
                        'JUMLAH_SATUAN' => $r['DLVSCR_BB_ITMQT'],
            ...
            foreach ($tpb_barang as &$n) {
                $tpb_barang_tarif = [
                    [
                        'JENIS_TARIF' => 'BM', 'KODE_FASILITAS' => 0, 'KODE_TARIF' => 1, 'NILAI_BAYAR' => 0, 'NILAI_FASILITAS' => 0, 'NILAI_SUDAH_DILUNASI' => 0, 'SERI_BARANG' => $n['SERI_BARANG'], 'TARIF' => $n['RBM'], 'TARIF_FASILITAS' => 100,
                    ],
                    [
                        'JENIS_TARIF' => 'PPN', 'KODE_FASILITAS' => 0, 'KODE_TARIF' => 1, 'NILAI_BAYAR' => 0, 'NILAI_FASILITAS' => 0, 'NILAI_SUDAH_DILUNASI' => 0, 'SERI_BARANG' => $n['SERI_BARANG'], 'TARIF' => 11, 'TARIF_FASILITAS' => 100,
                    ],
                    [
                        'JENIS_TARIF' => 'PPH', 'KODE_FASILITAS' => 0, 'KODE_TARIF' => 1, 'NILAI_BAYAR' => 0, 'NILAI_FASILITAS' => 0, 'NILAI_SUDAH_DILUNASI' => 0, 'SERI_BARANG' => $n['SERI_BARANG'], 'TARIF' => 2.5, 'TARIF_FASILITAS' => 100,
                    ],
                ];
    ...
    public function postingCEISA40BC41rm()
    {
        header('Content-Type: application/json');
        $doc = $this->input->post('doc');
        $RSHeader = $this->DELV_mod->selectDocument(['DLV_ID', 'DLV_BCDATE', 'RTRIM(ISNULL(MCUS_CURCD,MSUP_SUPCR)) MCUS_CURCD', 'DLV_ZNOMOR_AJU', 'DLV_PURPOSE', 'DLV_ITMCD'], ['DLV_ID' => $doc]);
        $data = [];
        $message = null;
        $isScrapItem = false;
        $itemCodeBarang = '';
    ...
    public function postingCEISA40BC25()
    {
        header('Content-Type: application/json');
        $doc = $this->input->post('doc');
    ...
    public function postingCEISA40BC41()
    {
        header('Content-Type: application/json');
        $doc = $this->input->post('doc');
        $RSHeader = $this->DELV_mod->selectDocument(['DLV_ID', 'DLV_BCDATE', 'RTRIM(MCUS_CURCD) MCUS_CURCD', 'DLV_ZNOMOR_AJU'], ['DLV_ID' => $doc]);
