...
class SPL extends CI_Controller
{
                                    ...
                                    if ($d['USED'] == false) {
                                        if (count($rsfix) == 0) {
                                            array_push(
                                                $rsfix,
                                                array(
                                                    "SPLSCN_ID" => trim($d["SPLSCN_ID"]), "SPLSCN_USRID" => trim($d["SPLSCN_USRID"]),
    ...
    public function printresult()
    {
        ...
        if (substr($cpsn, 0, 3) == 'PR-') {
            ...
            foreach ($rshead as $r) {
                $cprocess = trim($r['PPSN1_PROCD']);
                $cissu = date_create($r['PPSN1_ISUDT']);
                break;
            }
    ...
    public function book_closing()
    {
        ...
        $myar = [];
        $ret1 = $this->SPLBOOK_mod->updatebyVars(
            ['SPLBOOK_QTY' => $issuedQTY, 'SPLBOOK_CLOSEDDT' => $currentDateTime],
