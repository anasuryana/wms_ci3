...
class User extends CI_Controller
{
    ...
    public function form_policy()
    {
        $this->load->view('UserMGR/vpassword_policy');
    }
    public function form_lock_policy()
    {
    ...
    public function set_password_policy()
    {
        header('Content-Type: application/json');
    ...
    public function getName($puser = null)
    {
        header('Content-Type: application/json');
        if ($puser != '-') {
    ...
    public function setgroup()
    {
        ...
        if (strlen(trim($cid)) === 0) {
            die(json_encode([['cd' => 0, 'dsc' => 'ID is required']]));
        }
