...
class DELV_mod extends CI_Model {
    ...
    {	
        ...
        isnull(DLV_BCTYPE,'-') DLV_BCTYPE,ISNULL(DLV_NOAJU,'') DLV_NOAJU,DLV_NOPEN,COALESCE(DLV_DESTOFFICE,'-') DLV_DESTOFFICE,
	...
	{	
        ...
        post.MSTEMP_FNM DLV_POST,DLV_POSTTM,isnull(DLV_BCTYPE,'-') DLV_BCTYPE,ISNULL(DLV_NOAJU,'') DLV_NOAJU ,DLV_NOPEN,COALESCE(DLV_DESTOFFICE,'-') DLV_DESTOFFICE,DLV_PURPOSE,DLV_BSGRP,ISNULL(DLV_CUSTDO,'') DLV_CUSTDO
        ,ISNULL(DLV_ZJENIS_TPB_ASAL,'-') DLV_ZJENIS_TPB_ASAL, ISNULL(DLV_ZJENIS_TPB_TUJUAN,'-') DLV_ZJENIS_TPB_TUJUAN,DLV_BCDATE
        ,isnull(DLV_ZSKB,'') DLV_ZSKB,isnull(DLV_ZKODE_CARA_ANGKUT,'-') DLV_ZKODE_CARA_ANGKUT,DLV_ZTANGGAL_SKB,ISNULL(DLV_CONA,'') DLV_CONA,DLV_RPDATE,MAX(SI_WH) SI_WH,ISNULL(MAX(DLV_SPPBDOC),'') DLV_SPPBDOC");
		$this->db->from($this->TABLENAME." a");
        $this->db->join("MCUS_TBL b", "a.DLV_CUSTCD=b.MCUS_CUSCD"); 
	...
	{	
        ...
        ISNULL(DLV_NOAJU,'') DLV_NOAJU,DLV_NOPEN,COALESCE(DLV_DESTOFFICE,'-') DLV_DESTOFFICE,DLV_PURPOSE,DLV_BSGRP,ISNULL(DLV_CUSTDO,'') DLV_CUSTDO
        ,ISNULL(DLV_ZJENIS_TPB_ASAL,'-') DLV_ZJENIS_TPB_ASAL, ISNULL(DLV_ZJENIS_TPB_TUJUAN,'-') DLV_ZJENIS_TPB_TUJUAN,DLV_BCDATE
        ,isnull(DLV_ZSKB,'') DLV_ZSKB,isnull(DLV_ZKODE_CARA_ANGKUT,'-') DLV_ZKODE_CARA_ANGKUT,DLV_ZTANGGAL_SKB,ISNULL(DLV_CONA,'') DLV_CONA,DLV_RPDATE,MAX(SI_WH) SI_WH,ISNULL(MAX(DLV_SPPBDOC),'') DLV_SPPBDOC");
		$this->db->from($this->TABLENAME." a");
        $this->db->join("MCUS_TBL b", "a.DLV_CUSTCD=b.MCUS_CUSCD"); 
    ...
    public function select_det_rm_byid($pid){
        ...
        LEFT JOIN 
        (SELECT ITH_ITMCD,ITH_DOC FROM ITH_TBL WHERE ITH_WH='ARWH0PD' AND ITH_FORM='OUT-SHP-RM'
    ...
    public function select_bom($pdo){    
        // $qry = "SELECT SER_ITMID,SERD2_FGQTY,SERD2_ITMCD,VX.SERD2_SER,VX.SERD2_JOB,SERD2_QTPER MYPER FROM
        // DLV_TBL LEFT JOIN
        //     (
        //     SELECT SERD2_PSNNO,SERD2_JOB,SERD2_LINENO,SERD2_CAT,SERD2_FR,SERD2_MC,SERD2_FGQTY,SERD2_MCZ,SERD2_QTPER,SUM(SERD2_QTY) SERD2_QTYSUM,SERD2_ITMCD,SERD2_SER FROM SERD2_TBL
        //     GROUP BY SERD2_PSNNO,SERD2_JOB,SERD2_LINENO,SERD2_CAT,SERD2_FR,SERD2_MC,SERD2_MCZ,SERD2_QTPER,SERD2_ITMCD,SERD2_FGQTY,SERD2_SER) VX 
        //     ON DLV_SER=SERD2_SER
        //     LEFT JOIN SER_TBL ON SERD2_SER=SER_ID
        //     WHERE DLV_ID=?
        //     ORDER BY DLV_SER, SERD2_ITMCD";
        // $qry = "SELECT SER_ITMID,SERD2_FGQTY,SERD2_ITMCD,VX.SERD2_SER,SER_DOC,SERD2_QTPER MYPER,MITM_MODEL FROM
        // DLV_TBL LEFT JOIN
        //     (
        //     SELECT SERD2_FGQTY,SUM(SERD2_QTPER) SERD2_QTPER,SUM(SERD2_QTY) SERD2_QTYSUM,SERD2_ITMCD,SERD2_SER FROM SERD2_TBL
		// 	where SERD2_SER in (select DLV_SER from DLV_TBL WHERE DLV_ID=?)
        //     GROUP BY SERD2_QTPER,SERD2_ITMCD,SERD2_FGQTY,SERD2_SER) VX 
        //     ON DLV_SER=SERD2_SER
        //     LEFT JOIN SER_TBL ON SERD2_SER=SER_ID
	...
	{	
        ...
        ,isnull(DLV_PURPOSE,'-') DLV_PURPOSE,DLV_DESTOFFICE
    ...
    public function select_serah_terima_asp($psi){
        $qry = "SELECT V1.*,SCAN_DATE,MBSG_DESC,MCUS_CUSNM,SI_HRMRK,RTRIM(MITM_ITMD1) MITM_ITMD1,TTLBOX*MITM_NWG MITM_NWG, TTLBOX* MITM_BOXWEIGHT MITM_GWG,1 TTLBARIS  FROM
    ...
    public function select_for_do_rm_rtn_v1($pdoc){
        $qry = "select DLVRMDOC_ITMID,DLVRMDOC_AJU,DLVRMDOC_NOPEN,sum(DLVRMDOC_ITMQT) DLVRMDOC_ITMQT
        ,RTRIM(MITM_ITMD1) MITM_ITMD1,DLV_ITMD1,RTRIM(MITM_SPTNO) MITM_SPTNO,DLV_ITMSPTNO,DLV_DATE,MDEL_ZNAMA,DLV_DSCRPTN 
        ,DLV_BCTYPE,DLV_INVNO,DLV_NOPEN,MDEL_ADDRCUSTOMS,RTRIM(MITM_STKUOM) MITM_STKUOM,DLVRMDOC_DO
        ,ISNULL(MAX(MITM_ITMCDCUS),'') MITM_ITMCDCUS,DLVRMDOC_TYPE from DLVRMDOC_TBL 
        LEFT JOIN ( SELECT DLV_ID,MDEL_ZNAMA,MDEL_ADDRCUSTOMS,DLV_DATE,ISNULL(DLV_INVNO,'') DLV_INVNO 
                ...
                ,max(DLV_DSCRPTN) DLV_DSCRPTN,max(DLV_BCTYPE) DLV_BCTYPE
                ...
                ) VH on DLVRMDOC_ITMID=DLV_ITMCD and DLVRMDOC_TXID=DLV_ID
        LEFT JOIN MITM_TBL ON DLVRMDOC_ITMID=MITM_ITMCD
        where DLVRMDOC_TXID=?
        group by DLVRMDOC_ITMID,DLVRMDOC_AJU,DLVRMDOC_NOPEN,MITM_ITMD1,DLV_ITMD1,MITM_SPTNO,DLV_ITMSPTNO,DLV_DATE,MDEL_ZNAMA,DLV_DSCRPTN,DLV_INVNO
        ,DLV_BCTYPE,DLV_NOPEN,MDEL_ADDRCUSTOMS,MITM_STKUOM,DLVRMDOC_DO,DLVRMDOC_TYPE
        ORDER BY DLVRMDOC_ITMID,DLVRMDOC_AJU";
        $query = $this->db->query($qry, [$pdoc]);
        return $query->result_array();
    }
    ...
    public function select_for_pkg_rm_rtn($pdoc){
        ...
        ,DLV_SMTINVNO,MCUS_CURCD,DLV_INVDT,DLV_NOPEN,DLV_ITMD1,DLV_ITMSPTNO,RTRIM(MITM_SPTNO) MITM_SPTNO,ISNULL(MITM_ITMCDCUS,'-') MITM_ITMCDCUS FROM DLV_PKG_TBL
