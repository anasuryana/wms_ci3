...
class SERRC_mod extends CI_Model
{
    ...
    public function select_xbalance_v1($pqalabel, $pdoc)
    { //hybrid
    ...
    {
        $qry = "select SERRC_SER,count(*) OUTQTY,SER_DOC,SERRC_SERX,SER_ITMID,SERRC_NASSYCD,RTRIM(MAX(STKTRND1H_LOCCDFR)) STKTRND1_LOCCDFR from
		...
		left join SER_TBL ON SERRC_SER=SER_ID
        LEFT JOIN (select MAX(STKTRND1H_BSGRP) STKTRND1H_BSGRP,MAX(STKTRND1H_LOCCDFR) STKTRND1H_LOCCDFR,STKTRND1H_DOCNO FROM XSTKTRND1H GROUP BY STKTRND1H_DOCNO) V ON SER_DOC=STKTRND1H_DOCNO
