...
class ITH_mod extends CI_Model
{
    ...
    public function selectGroupByDocumentAndWarehouse($where) {
        $this->db->from($this->TABLENAME);
        $this->db->select('ITH_DOC,ITH_ITMCD,ITH_WH, sum(ITH_QTY) TTLQT');
        $this->db->where($where);
        $this->db->group_by('ITH_DOC,ITH_ITMCD,ITH_WH');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function insert_cancel_kitting_out($data)
    {
        ...
        $this->db->query($qry, array(
            ...
            $data['ITH_DATE'], $data['ITH_USRID'],  $data['ITH_SER']
        ));
        return $this->db->affected_rows();
    }
    ...
    {
        $qry = "SELECT RTRIM(RPSTOCK_ITMNUM) RPSTOCK_ITMNUM,RPSTOCK_NOAJU,RPSTOCK_BCNUM,RPSTOCK_QTY BALQTY,RTRIM(MITM_ITMD1) MITM_ITMD1,RTRIM(RPSTOCK_DOC) RPSTOCK_DOC,FORMAT(IODATE,'dd-MMM-yy') INCDATE FROM ZRPSAL_BCSTOCK
    ...
    {
        $qry = "SELECT RTRIM(RPSTOCK_ITMNUM) RPSTOCK_ITMNUM,RPSTOCK_NOAJU,RPSTOCK_BCNUM,SUM(RPSTOCK_QTY) BALQTY,RTRIM(MITM_ITMD1) MITM_ITMD1,RTRIM(RPSTOCK_DOC) RPSTOCK_DOC,FORMAT(MIN(IODATE),'dd-MMM-yy') INCDATE FROM ZRPSAL_BCSTOCK
        LEFT JOIN MITM_TBL ON RPSTOCK_ITMNUM=MITM_ITMCD
    ...
    public function select_txhistory_customs($plike, $pdate)
    {
        ...
        ISNULL(SUM(CASE WHEN RPSTOCK_QTY<0  THEN RPSTOCK_QTY END),0) OUTQTY,'' MITM_ITMD1,RPSTOCK_REMARK DOC,FORMAT(IODATE,'dd-MMM-yy') IODATE, 0 BAL,0 HEADER, '' AJU,RTRIM(RPSTOCK_DOC) RPSTOCK_DOC , '' MUSED
    ...
    {
        $qry = "SELECT ITH_DATEC,ITH_DOC,DLV_CUSTDO,DLV_CONSIGN,NOMAJU,NOMPEN,INVDT,DLV_INVNO,DLV_SMTINVNO,ITH_ITMCD,RTRIM(MITM_ITMD1) ITMDESCD,DLVPRC_CPO,DLVPRC_QTY,DLVPRC_PRC,(DLVPRC_QTY*DLVPRC_PRC) AMOUNT,DLV_SMTINVNO,DLV_SPPBDOC,DLV_CUSTCD FROM
        (SELECT ITH_DATEC,ITH_ITMCD,ITH_DOC FROM v_ith_tblc      
                where (ITH_DATEC between ? and ?) and ITH_WH IN ('ARSHP','ARSHPRTN2') AND ITH_FORM='OUT-SHP-FG'        
                GROUP BY ITH_ITMCD,ITH_DOC,ITH_DATEC		
        ) V1
        LEFT JOIN 
        (
        SELECT DLV_ID,max(DLV_CONSIGN) DLV_CONSIGN,max(DLV_INVDT) INVDT,MAX(DLV_INVNO) DLV_INVNO, MAX(DLV_SMTINVNO) DLV_SMTINVNO
                    ,MAX(DLV_ZNOMOR_AJU) NOMAJU,max(DLV_NOPEN) NOMPEN,MAX(DLV_CUSTDO) DLV_CUSTDO,MAX(DLV_SPPBDOC) DLV_SPPBDOC,MAX(DLV_CUSTCD) DLV_CUSTCD 
        FROM DLV_TBL WHERE DLV_BSGRP IN ($bigrup)
        GROUP BY DLV_ID
        ) V2 ON ITH_DOC=DLV_ID
        LEFT JOIN MITM_TBL ON ITH_ITMCD=MITM_ITMCD
        LEFT JOIN
            (
        ...
        (select DLV_DATE,DLV_ID,DLV_CUSTDO,DLV_CONSIGN,DLV_ZNOMOR_AJU NOMAJU,
            ...
            DLV_ZJENIS_TPB_TUJUAN,DLV_RPDATE TGLPEN,DLV_PURPOSE,MAX(DLV_SPPBDOC) DLV_SPPBDOC,MAX(DLV_CUSTCD) DLV_CUSTCD 
            from DLV_TBL
        ...
        INNER join (
            ...
            group by DLVRMSO_TXID,DLVRMSO_ITMID,MITM_ITMD1,DLVRMSO_PRPRC,MITM_STKUOM,DLVRMSO_CPO) vselect on DLV_ID=DLVRMSO_TXID
    ...
    public function select_parent_locations(): array
    {
        ...
        group by FTRN_LOCCD
        UNION
        SELECT RTRIM(MSTLOCG_ID) LOCATIONCD FROM MSTLOCG_TBL WHERE MSTLOCG_ID like '%EQUIP%'
