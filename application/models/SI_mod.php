...
class SI_mod extends CI_Model {
    ...
    public function select_outgoing_wh_byassy_bg($pbg, $pdtfrom, $pdtto, $passy){
        // $qry = "SELECT SI_ITMCD,MITM_ITMD1,SI_DOC,isnull(SISCN_SERQTY,0) SISCN_SERQTY,SER_DOC,SISCN_SER,SI_DOCREFFETA,SISCN_LUPDT,SI_BSGRP,DLV_ID, ISNULL(SI_OTHRMRK,SI_HRMRK) SI_OTHRMRK FROM SI_TBL 
        ...
        $qry = "SELECT SI_ITMCD,MITM_ITMD1,SI_DOC,isnull(SISCN_SERQTY,0) SISCN_SERQTY,SER_DOC,SISCN_SER,SI_DOCREFFETA,SISCN_LUPDT,SI_BSGRP,DLV_ID, ISNULL(SI_OTHRMRK,SI_HRMRK) SI_OTHRMRK,ITH_LUPDT FROM SI_TBL 
        LEFT JOIN MITM_TBL ON SI_ITMCD=MITM_ITMCD
        LEFT JOIN SISCN_TBL ON SI_LINENO=SISCN_LINENO
        LEFT JOIN SER_TBL ON SISCN_SER=SER_ID
        LEFT JOIN DLV_TBL ON SISCN_SER=DLV_SER
        LEFT JOIN (SELECT ITH_SER,ITH_LUPDT FROM ITH_TBL WHERE ITH_FORM='OUT-SHP-FG' and ITH_WH in ('ARSHP','ARSHPRTN')) VSHP ON DLV_SER=ITH_SER
        WHERE SI_QTY>0 AND SI_BSGRP=? AND (SI_LUPDT BETWEEN ? AND ? )
        AND SI_ITMCD LIKE ?
        order by SI_DOCREFFETA, SISCN_LUPDT";
