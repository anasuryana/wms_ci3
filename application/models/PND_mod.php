...
class PND_mod extends CI_Model {
    ...
    public function selectscan_balancingv2($pdo){        
		...
		FROM PND_TBL a 
                LEFT JOIN PNDSCN_TBL b ON a.PND_DOC=b.PNDSCN_DOC and a.PND_ITMCD=b.PNDSCN_ITMCD  AND PND_ITMLOT=PNDSCN_LOTNO             
                WHERE PND_DOC='".$pdo."'  
                GROUP BY PND_DOC, PND_ITMCD,PND_QTY
                ) v1
        LEFT JOIN (
        SELECT ITH_ITMCD,COALESCE(sum(ITH_QTY),0) ITHQTY FROM ITH_TBL 
        WHERE ITH_DOC='".$pdo."' AND ITH_FORM='INC-PEN-RM'
        GROUP BY ITH_DOC, ITH_ITMCD) v2 ON v1.PND_ITMCD=v2.ITH_ITMCD";
        $query = $this->db->query($qry);
		return $query->result_array();
    }

    public function selectprog_scan($pdo){
        $qry = "exec xsp_progress_scnpnd '$pdo'";
		$resq = $this->db->query($qry);
		return $resq->result_array();
    }

    public function selectprog_save($pdo){
        $qry = "exec xsp_progress_scnpndsave '$pdo'";
		$resq = $this->db->query($qry);
		return $resq->result_array();
    }
