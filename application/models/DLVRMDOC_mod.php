...
class DLVRMDOC_mod extends CI_Model {
    ...
    public function select_invoice($ptxid){
        ...
        (select DLVRMDOC_TXID,DLVRMDOC_ITMID,RTRIM(MITM_ITMD1) MITM_ITMD1, DLVRMDOC_PRPRC,sum(DLVRMDOC_ITMQT) ITMQT, DLVRMDOC_PRPRC* SUM(DLVRMDOC_ITMQT) AMNT ,RTRIM(MITM_STKUOM) MITM_STKUOM
        from DLVRMDOC_TBL LEFT JOIN MITM_TBL ON DLVRMDOC_ITMID=MITM_ITMCD
        WHERE DLVRMDOC_TXID=?
        group by DLVRMDOC_ITMID,MITM_ITMD1, DLVRMDOC_PRPRC,DLVRMDOC_TXID,MITM_STKUOM) VINV
        LEFT JOIN (
