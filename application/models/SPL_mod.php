...
class SPL_mod extends CI_Model {
    ...
    public function selectby4par($pspl, $pcat, $pline, $pfr){
        ...
        $this->db->join("(SELECT SPLSCN_ORDERNO,SPLSCN_ITMCD,SUM(SPLSCN_QTY) TTLSCN 
        ...
        "a.SPL_ITMCD=c.SPLSCN_ITMCD AND a.SPL_ORDERNO=c.SPLSCN_ORDERNO", 'left');  
    ...
    public function select_compare_psnjob_req($pdocno, $pwo){
        ...
        if($pwo=='22-5A09-217724501') {
            ...
            ,RTRIM(MITM_ITMD1) MITM_ITMD1,rtrim(MITM_SPTNO) MITM_SPTNO,SIMQT FROM XMBOM
    ...
    public function select_psnjob_req($pdocno, $pwo){
        ...
        // SO WE NEED TO HANDLE THIS PROBLEM
        // NOTE : ASK PPIC DEPT FOR DETAIL PROBLEM
        $specialWO = ['22-5A09-217724501'];
        if(in_array($pwo, $specialWO)) {
            $qry = "SELECT PDPP_MDLCD,PDPP_WONO PIS3_WONO, MBOM_LINE PIS3_LINENO, '' PIS3_FR, '' PIS3_PROCD, '' PIS3_MC, '' PIS3_MCZ, PDPP_WORQT*MBOM_PARQT PIS3_REQQTSUM, MBOM_PARQT MYPER, MBOM_BOMPN PIS3_ITMCD, MBOM_BOMPN PIS3_MPART  FROM XMBOM
            inner join XWO ON MBOM_MDLCD=PDPP_MDLCD AND MBOM_BOMRV=PDPP_BOMRV
            WHERE PDPP_WONO=?";
            $query = $this->db->query($qry, [$pwo] );
        } else {
