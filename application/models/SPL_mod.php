...
class SPL_mod extends CI_Model
{
    ...
    public function selectWOOpen_wip($pwo)
    {
        $qry = "select XWO.*, CONVERT(bigint,coalesce(LBLTTL,0)) LBLTTL,MITM_ITMD1,PDPP_BSGRP,PDPP_CUSCD,MITM_LBLCLR,COALESCE(MITM_SHTQTY,0) MITM_SHTQTY,MITM_SPQ from
        XWO LEFT JOIN MITM_TBL ON MITM_ITMCD=PDPP_MDLCD
        LEFT JOIN
    ...
    {
        $this->db->limit(1);
        $this->db->select("SPL_DOC,SPL_CAT, SPL_LINE, SPL_FEDR, SPL_ORDERNO,SPL_ITMCD, convert(int,TTLREQ) TTLREQ, convert(int,COALESCE(TTLSCN,0)) TTLSCN,SPL_PROCD ");
        $this->db->from(
            "(SELECT SPL_DOC,SPL_CAT, SPL_LINE, SPL_FEDR, SPL_ORDERNO,SPL_ITMCD, sum(SPL_QTYREQ) TTLREQ,SPL_PROCD from $this->TABLENAME
            where SPL_DOC = '$pspl' AND  SPL_CAT='$pcat' AND SPL_LINE='$pline' AND SPL_FEDR='$pfr' AND SPL_ITMCD='$pitem'
            GROUP BY SPL_DOC,SPL_CAT,SPL_LINE,SPL_FEDR,SPL_ORDERNO,SPL_ITMCD,SPL_PROCD) a"
        );
        $this->db->join(
            "(select SPLSCN_DOC,SPLSCN_CAT,SPLSCN_LINE,SPLSCN_FEDR,SPLSCN_ORDERNO,SPLSCN_ITMCD, sum(SPLSCN_QTY) TTLSCN,SPLSCN_PROCD from SPLSCN_TBL
            where SPLSCN_DOC = '$pspl' AND  SPLSCN_CAT='$pcat' AND SPLSCN_LINE='$pline' AND SPLSCN_FEDR='$pfr' and SPLSCN_ITMCD='$pitem'
            GROUP BY SPLSCN_DOC,SPLSCN_CAT,SPLSCN_LINE,SPLSCN_FEDR,SPLSCN_ORDERNO,SPLSCN_ITMCD,SPLSCN_PROCD) v1",
            "a.SPL_DOC=v1.SPLSCN_DOC and a.SPL_CAT=v1.SPLSCN_CAT and a.SPL_LINE=v1.SPLSCN_LINE and a.SPL_FEDR=v1.SPLSCN_FEDR and a.SPL_ORDERNO=v1.SPLSCN_ORDERNO and a.SPL_ITMCD=v1.SPLSCN_ITMCD AND a.SPL_PROCD=v1.SPLSCN_PROCD",
            "left"
        );
    ...
    public function select_compare_psnjob_req($pdocno, $pwo)
    {
        ...
        if (in_array(strtoupper($pwo), $this->SPECIALWO)) {
            ...
            WHERE PDPP_WONO=?
            GROUP BY MBOM_LINE,PDPP_WORQT,MBOM_PARQT,MBOM_BOMPN,SIMQT,MITM_ITMD1,MITM_SPTNO";
    ...
    public function select_partreq_h_bypart($pPart, $period1, $period2)
    {
        $qry = "SELECT V1.*,RQSRMRK_DESC FROM
                (SELECT TOP 200 SPL_DOC
                            ,max(SPL_LUPDT) DT
                        ...
                        WHERE SPL_DOC LIKE 'PR-%'
                            AND SPL_ITMCD LIKE ?
                            AND SPL_LUPDT BETWEEN ?
                                AND ?
                        GROUP BY SPL_DOC ) V1
                        LEFT JOIN RQSRMRK_TBL ON SPL_RMRK=RQSRMRK_CD
                ";
        $query = $this->db->query($qry, ['%' . $pPart . '%', $period1, $period2]);
        return $query->result_array();
    }
