...
class PO_mod extends CI_Model {
    ...
    public function select_header_where($pwhere){
        $this->db->select("PO_NO,PO_RMRK,PO_PPH,PO_VAT,PO_SUPCD,MSUP_SUPNM,MSUP_SUPCR,PO_RQSTBY,PO_PAYTERM,PO_REQDT,PO_DLVDT,PO_SHPDLV,PO_SHPCOST,MAX(PO_DEPT) PO_DEPT");
        $this->db->from($this->TABLENAME);
        $this->db->where($pwhere);
        ...
        $this->db->group_by("PO_NO,PO_RMRK,PO_PPH,PO_VAT,PO_SUPCD,MSUP_SUPNM,MSUP_SUPCR,PO_RQSTBY,PO_PAYTERM,PO_REQDT,PO_DLVDT,PO_SHPDLV,PO_SHPCOST");
		$query = $this->db->get();
		return $query->result_array();
    }
    ...
    public function select_header_period_like($pdate0,$pdate1,$plike){
        $this->db->select("PO_NO,PO_RMRK,PO_PPH,PO_VAT,PO_SUPCD,MSUP_SUPNM,MSUP_SUPCR,PO_RQSTBY,PO_PAYTERM,PO_REQDT,PO_DLVDT,PO_SHPDLV,PO_SHPCOST,MAX(PO_DEPT) PO_DEPT");
        $this->db->from($this->TABLENAME);
        $this->db->like($plike)->where("PO_REQDT>=", $pdate0 )->where("PO_REQDT<=", $pdate1);
        $this->db->join("(select MSUP_SUPCD,rtrim(MAX(MSUP_SUPNM)) MSUP_SUPNM,MAX(MSUP_SUPCR) MSUP_SUPCR FROM v_supplier_customer_union GROUP BY MSUP_SUPCD) V1", "PO_SUPCD=MSUP_SUPCD", "left");
        $this->db->group_by("PO_NO,PO_RMRK,PO_PPH,PO_VAT,PO_SUPCD,MSUP_SUPNM,MSUP_SUPCR,PO_RQSTBY,PO_PAYTERM,PO_REQDT,PO_DLVDT,PO_SHPDLV,PO_SHPCOST");
        $this->db->order_by("PO_REQDT");
		$query = $this->db->get();
    ...
    public function select_detail_where($pwhere){
        ...
        $this->db->where($pwhere);
        $this->db->order_by("PO_DISC DESC, PO_ITMNM, MITM_ITMD1");
		$query = $this->db->get();
		return $query->result_array();
    }
    ...
    public function select_balance_nonitem_like($plike){
        $this->db->select($this->TABLENAME.".*, RTRIM(PO_ITMNM) MITM_ITMD1,PO_UM MITM_STKUOM,MSUP_SUPNM,MSUP_SUPCR,MSUP_ADDR1,MSUP_TELNO,MSUP_FAXNO,ISNULL(RCVQTY,0) RCVQTY");
        $this->db->from($this->TABLENAME);
        $this->db->join("MSUP_TBL", "PO_SUPCD=MSUP_SUPCD", "LEFT");
        $this->db->join("(SELECT RCV_PO,RCV_ITMCD,SUM(RCV_QTY) RCVQTY FROM RCV_TBL GROUP BY RCV_PO,RCV_ITMCD) VRCV", "PO_NO=RCV_PO AND PO_ITMCD=RCV_ITMCD", "LEFT");
    ...
    public function select_report($pdate0,$pdate1){
        ...
        (select PO_NO,CONVERT(DATE,PO_CRTDT) PO_CRTDT,PO_SUPCD,PO_RMRK,PO_SHPDLV,PO_PAYTERM,PO_PPH,PO_VAT,SUM(PO_QTY)*PO_PRICE AMOUNT, PO_ITMCD,RTRIM(MITM_ITMD1) MITM_ITMD1, '' ITMGROUP 
        ...
        where (PO_REQDT BETWEEN ? AND ? ) AND PO_ITMCD IS NOT NULL
        GROUP BY PO_NO,PO_CRTDT,PO_SUPCD,PO_RMRK,PO_SHPDLV,PO_PAYTERM,PO_PPH,PO_VAT,PO_PRICE, PO_ITMCD,MITM_ITMD1,MITM_STKUOM) VPOITEM
        LEFT JOIN
        (
        ...
        (select PO_NO,PO_CRTDT,PO_SUPCD,PO_RMRK,PO_SHPDLV,PO_PAYTERM,PO_PPH,PO_VAT,SUM(PO_QTY)*PO_PRICE AMOUNT, 'NON ITEM' PO_ITMCD,PO_ITMNM MITM_ITMD1, '' ITMGROUP 
        ,SUM(PO_QTY) TQTY,PO_UM MITM_STKUOM,PO_PRICE,SUM(PO_DISC) PO_DISC
        ,(SUM(PO_QTY)*PO_PRICE)-SUM(PO_QTY)*PO_PRICE*SUM(PO_DISC)/100 NETAMOUNT from PO_TBL 
        where (PO_REQDT BETWEEN ? AND ? ) AND PO_ITMCD IS NULL
        GROUP BY PO_NO,PO_CRTDT,PO_SUPCD,PO_RMRK,PO_SHPDLV,PO_PAYTERM,PO_PPH,PO_VAT,PO_PRICE, PO_ITMNM,PO_UM) VPOITEM
        LEFT JOIN
        (
