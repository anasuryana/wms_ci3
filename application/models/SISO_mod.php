...
class SISO_mod extends CI_Model {
    ...
    public function select_currentPlot($pdoc){
        $qry = "SELECT SISO_HLINE,SSO2_SLPRC,SUM(SISO_QTY) PLOTQTY,SISO_SOLINE,SISO_CPONO,MAX(SCNQT) SCNQT,RTRIM(MAX(ITMID)) ITMID,MAX(SSO2_BSGRP) SSO2_BSGRP,
        max(RTRIM(SSO2_CUSCD)) SSO2_CUSCD
        FROM SISO_TBL
