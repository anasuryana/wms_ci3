...
const devNode = $('#mmenu').tree({
    ...
    onLoadSuccess: function(node, data) {
        const myfind = $(this).tree('findBy', {field: 'id', value: 'CL'})
		// getmessageFlag(myfind)
    }
})

function getmessageFlag(ptarget) {    
	// devNode.tree('update', {target: ptarget.target, text: `Abnormal <i class="fas fa-spinner fa-pulse"></i>`})  
    // $.ajax({        
    //     url: "<?//=base_url('MSG/getbyUser')?>",
    //     dataType: "json",
    //     success: function (response) {    
    //         if(response[0].TTLMSG>0) {
    //             devNode.tree('update', {target: ptarget.target, text: `Abnormal <span class="badge bg-danger">${response[0].TTLMSG}</span>`})
    //         } else {
    //             devNode.tree('update', {target: ptarget.target, text: `Abnormal`})
    //         }            
    //     }
    // });	
}
...
function addT(Judul,alamat){
    $.post("<?=base_url('Pages/l')?>", {inmenu : Judul, inurl: alamat},
...
function showinfoofus(){
    let mymodal = new bootstrap.Modal(document.getElementById("FOOTER_MODWH"), {backdrop: 'static', keyboard: false});
    mymodal.show();
    // $("#FOOTER_MODWH").modal({backdrop: 'static', keyboard: false});    
}

function showinfoofus_FG(){
    let mymodal = new bootstrap.Modal(document.getElementById("FOOTER_MODWHFG"), {backdrop: 'static', keyboard: false});
    mymodal.show();
    // $("#FOOTER_MODWHFG").modal({backdrop: 'static', keyboard: false});       
}
$("#FOOTER_MODWHFG_btnsave").click(function (e) { 
    ...
    $("#FOOTER_MODWHFG").modal('hide'); alertify.message('Saved');
});
$("#FOOTER_MODWH_btnsave").click(function (e) { 
    ...
    $('#tt').tabs({
        ...
        onSelect: function(title, index){       
            if(title=='Pending RM Scan' ||  title=='Analyze Returned RM' || 
