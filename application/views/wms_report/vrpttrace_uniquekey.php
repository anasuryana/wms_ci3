<div style="padding: 5px">
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-12 mb-1">
                <div class="table-responsive" id="vtraceuniq_divku_shp">
                    <table id="vtraceuniq_tbl_shp" class="table table-striped table-bordered table-sm table-hover caption-top" style="font-size:95%">
                        <caption>Shipping Instruction</caption>
                        <thead class="table-light">
                            <tr>
                                <th class="align-middle">SI Number</th>
                                <th class="align-middle text-center">ID</th>
                                <th class="align-middle text-end">Qty</th>
                                <th class="align-middle text-center">PIC</th>
                                <th class="align-middle text-center">Scanned Time</th>
                                <th class="align-middle text-center">Is saved ?</th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
                ...
                if (response.status[0].cd != '0') {
                    ...
                    if (!isfound) {
                        ...
                        newcell.onclick = function() {
                            vtraceuniq_get_childs(response.data[0].SER_ID)
                        };
                        ...
                        newcell.style.cssText = "cursor:pointer";
                        newcell.onclick = function() {
                            vtraceuniq_get_childs(response.data[0].SER_REFNO)
                        };
                        ...
                        if (response.data[0].SER_ID == response.data[0].SER_REFNO) {
                            ...
                            newcell.onclick = function() {
                                vtraceuniq_find(response.data[0].SER_REFNO)
                            };
                        }
                        newcell.appendChild(newText);
                        newcell = newrow.insertCell(7);
                        ...
                        newcell.onclick = function() {
                            vtraceuniq_e_showhistory(response.data[0].SER_ID)
                        };
                    }
                                ...
                                newcell.onclick = (e) => {
                                    if (confirm('Are you sure ?')) {
                                        ...
                                        $.ajax({
                                            ...
                                            data: {
                                                inunique: response.combine[i].SERC_COMID,
                    ...
                    ttlrows = response.si.length;
                    mydes = document.getElementById("vtraceuniq_divku_shp");
                    myfrag = document.createDocumentFragment();
                    mtabel = document.getElementById("vtraceuniq_tbl_shp");
                    cln = mtabel.cloneNode(true);
                    myfrag.appendChild(cln);
                    tabell = myfrag.getElementById("vtraceuniq_tbl_shp");
                    tableku2 = tabell.getElementsByTagName("tbody")[0]
                    tableku2.innerHTML = ''
                    for (let i = 0; i < ttlrows; i++) {
                        newrow = tableku2.insertRow(-1);
                        newcell = newrow.insertCell(0)
                        newcell.innerHTML = response.si[i].SISCN_DOC
                        newcell = newrow.insertCell(1)
                        newcell.classList.add('text-center')
                        newcell.innerHTML = response.si[i].SISCN_SER
                        newcell = newrow.insertCell(2)
                        newcell.classList.add('text-end')
                        newcell.innerHTML = response.si[i].SISCN_SERQTY
                        newcell = newrow.insertCell(3)
                        newcell.classList.add('text-center')
                        newcell.innerHTML = response.si[i].PIC
                        newcell = newrow.insertCell(4)
                        newcell.classList.add('text-center')
                        newcell.innerHTML = response.si[i].SISCN_LUPDT
                        newcell = newrow.insertCell(5)
                        if (response.si[i].SISCN_PLLT === '-') {
                            newcell.innerHTML = 'Not Saved'
                            newcell.classList.add('bg-warning','text-center')
                        } else {
                            newcell.innerHTML = 'Saved'
                            newcell.classList.add('bg-success', 'text-center','text-white')
                        }
                    }
                    mydes.innerHTML = ''
                    mydes.appendChild(myfrag)
            ...
            success: function(response) {
                ...
                if (response.status[0].cd != '0') {
                    ...
                    for (let i = 0; i < ttlrows; i++) {
                        ...
                        newcell.title = "get transaction history";
                        newcell.onclick = function() {
                            vtraceuniq_e_showhistory(response.data[i].SER_ID)
                        };
                    }
                    mydes.innerHTML = '';
                    mydes.appendChild(myfrag);
                    ...
                    if (ttlrows > 0) {
                        for (let i = 0; i < ttlrows; i++) {
                            ...
                            newcell.title = "get transaction history";
                            newcell.onclick = function() {
                                vtraceuniq_e_showhistory(response.data_multi[i].SERML_COMID)
                            };
                        }
                    } else {
                        alertify.message(response.status[0].msg);
