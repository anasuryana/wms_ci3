...
<script>
    ...
    function keikaku_load_line_code() {
        $.ajax({
            ...
            success: function (response) {
                ...
                mydes = document.getElementById("keikakuLineTblDiv");
                myfrag = document.createDocumentFragment();
                mtabel = document.getElementById("keikakuLineTbl");
                cln = mtabel.cloneNode(true);
                myfrag.appendChild(cln);
                tabell = myfrag.getElementById("keikakuLineTbl");
                checkBox = myfrag.getElementById("keikakuCheckAll");
    ...
    function keikaku_load_data() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(['E3','F3'].includes(keikaku_line_input.value)) {
                    response.data.forEach((arrayItem, index) => {
    ...
    function keikakuIsHW() {
        const hwLine = [
            'A3','B3','C3','D3','E3','F3','H3-1','H3-2','H3-3','J3-1','J3-2','K3','L3','M3','OFFLINE 3','OFFLINE 4', 'OFFLINE PS', 
            'PS2','PS3', 'ATH-1'
            ,'ATH-2','ATH-3','ATH-4','M3.1', 'M3.2'
        ]
        return hwLine.includes(keikaku_line_input.value) ? true : false
    }
