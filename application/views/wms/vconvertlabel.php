        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-1 pr-1">
                <div class="input-group input-group-sm mb-1">
                    ...
                    <input type="text" class="form-control" id="cvtlabel_oldlot" required readonly>
                </div>
            </div>
        </div>
        <div class="row">
...
<input type="hidden" id="cvtlabel_location" value="">
<script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status[0].cd == '1') {
                        cvtlabel_location.value = response.current_location
                        ...
                        document.getElementById('cvtlabel_oldlot').value = response.data[0].SER_LOTNO
    ...
    $("#cvtlabel_newreff").keypress(function(e) {
        if (e.which == 13) {
            ...
            if (['NFWH4RT', 'AFWH3RT'].includes(cvtlabel_location.value)) {
                const old_lot_number = cvtlabel_oldlot.value.toUpperCase()
                if (rawtext.includes('|')) {
                    const arawtext = rawtext.toUpperCase().split('|')
                    if (arawtext.length >= 3) {
                        const new_lot_number = arawtext[2].substring(2)
                        const new_item = arawtext[0].substring(2)
                        const new_qty = arawtext[3].substring(2)
                        const new_reff = arawtext[5].substring(2)
                        if (oldreff === new_reff) {
                            alertify.message('could not convert to same reff. number')
                            return
                        }
                        if (olditem === new_item) {
                            alertify.message('Please use relabel menu')
                            return
                        }
                        if (oldqty != new_qty) {
                            alertify.message('QTY must be same')
                            return
                        }
                        if (old_lot_number === new_lot_number) {
                            alertify.confirm(
                                `Confirmation`,
                                `Are you sure want to convert <b>${olditem}</b> to <b>${new_item}</b> ?`,
                                () => {
                                    cvtlabel_newreff.value = new_reff
                                    cvtlabel_save_regularlabel({
                                        inrawtext: rawtext,
                                        inremark: remark_nmod
                                    })                                    
                                },
                                () => {
                            ...
                            cvtlabel_newreff.value = ''
                            alertify.warning(`Lot number should be same`)
                        }
                    } else {
                        alertify.warning('Please enter valid IEI label !')
                    }
                } else {
                    alertify.warning('Please enter valid IEI label')
                }
    ...
    function cvtlabel_save_regularlabel(paramData) {
        cvtlabel_newreff.readOnly = true;
        $.ajax({
            type: "get",
            ...
            success: function(response) {
                if (response.status[0].cd != '0') {
                    ...
                    let newitem = ar[0].substr(2, ar[0].length - 2);
                    let newqty = ar[3].substr(2, ar[3].length - 2);
                    let remark_nmod = cvtlabel_txt_newmodel.value;
                    cvtlabel_newitem.value = newitem + remark_nmod;
                    cvtlabel_newqty.value = newqty;
                    cvtlabel_btn_save.focus();
                } else {
                    alertify.warning(response.status[0].msg);
                    cvtlabel_newreff.value = '';
                    cvtlabel_newreff.readOnly = false;
                }
            },
            error(xhr, xopt, xthrow) {
                alertify.error(xthrow);
            ...
            if (document.getElementById('cvtlabel_newreff').readOnly ||
                ...
                $.ajax({
                    ...
                    data: {
                        inoldreff: oldreff,
                        inoldjob: oldjob,
                        inolditemcd: olditemcd,
                        inoldqty: oldqty,
                        inrg_reff: rg_reff,
                        inrg_itemcd: rg_itemcd,
                        inrg_rawtxt: rg_rawtxt,
                        inrg_remark: rg_remark,
                        inka_itmcd: ka_itemcd,
                        inka_lotno: ka_lotno,
                        inka_reff: ka_reff,
                        inka_remark: ka_remark,
                        infgtype: fgtype
                    },
                    dataType: "json",
                    success: function(response) {
