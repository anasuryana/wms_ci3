            ...
            <div class="row">
                <div class="col mb-1">
                    <div class="input-group">                        
                        <button title="Download a Template File (*.xls File)" id="itmloc_btn_download" class="btn btn-outline-success btn-sm"><i class="fas fa-file-download"></i></button>
...
$('#itmloc_tblitm tbody').on( 'click', 'tr', function () { 
    ...
    $("#itmloc_txt_itemcd").val(mitem); $("#itmloc_txt_itmdsc1").val(mitemnm);
    $("#itmloc_txt_itmdsc2").val(mitemnm2);$("#itmloc_txt_spt").val(mspt);    
...
$('#itmloc_tbl tbody').on( 'click', 'tr', function () {
    ...
    if(itmloc_colidx==4){
        alertify.confirm('Need Confirmation', 'Are you sure ?'
            , function(){ 
                $.ajax({
                    type: "get",
                    url: "<?=base_url('ITMLOC/remove')?>",
                    data: {initem:mitm,inlocg: midlocg, inloc: midloc },
                    dataType: "text",
                    success: function (response) {
...
$("#itmloc_btn_add").click(function (e) { 
    ...
    const mitm = $("#itmloc_txt_itemcd").val();    
    if(mitm==''){
        $("#itmloc_txt_itemcd").focus();return;
    }
    ...
    $.ajax({
        ...
        data: {initem: mitm,inlocg: mitmlocg, inloc: mitmloc, inbg: business },
        dataType: "text",
        success: function (response) {
            alertify.message(response);
            itmloc_findloc();
...
$("#itmloc_btn_download").click(function (e) { 
    e.preventDefault();
    $.ajax({
        type: "get",
        url: "<?=base_url('ITMLOC/getlinkitemtemplate')?>",
        dataType: "text",                       
        success:function(response) {          
